...
<div id="wrapper">
  ...
  <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="card shadow mb-4">
              <div class="card-header"><h6 class="m-0 font-weight-bold text-primary">Data Profile</h6></div>
              <div class="card-body">
                <form id="profileForm" action="profile.php" method="post" enctype="multipart/form-data">
                  <div class="form-group">
                    <label>Username</label>
                    <input type="text" name="username" value="<?php echo htmlspecialchars($admin['username']); ?>" readonly class="form-control">
                  </div>
                  <div class="form-group">
                    <label>Nama</label>
                    <input type="text" name="nama" id="nama" value="<?php echo htmlspecialchars($admin['nama']); ?>" readonly class="form-control">
                  </div>
                  <div class="form-group">
                    <label>Email</label>
                    <input type="email" name="email" id="email" value="<?php echo htmlspecialchars($admin['email']); ?>" readonly class="form-control">
                  </div>
                  <div class="form-group">
                    <label>No WhatsApp</label>
                    <input type="text" name="wa" id="wa" value="<?php echo htmlspecialchars($admin['wa'] ?? ''); ?>" readonly class="form-control">
                  </div>

                  <div class="form-group">
                    <label>Upload Foto (JPG, max 500KB)</label>
                    <input type="file" name="foto" id="foto" accept=".jpg,.jpeg" class="form-control" disabled>
                  </div>

                  <div class="d-flex">
                    <button type="button" id="btnUbah" class="btn btn-primary mr-2">Ubah</button>
                    <button type="submit" id="btnSimpan" class="btn btn-success mr-2" disabled>Simpan</button>
                    <button type="button" id="btnBatal" class="btn btn-secondary" disabled>Batal</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          <div class="col-md-6">
            <div class="card shadow mb-4">
              <div class="card-header"><h6 class="m-0 font-weight-bold text-primary">Foto Profile</h6></div>
              <div class="card-body text-center">
                <img id="previewFoto" src="<?php echo (!empty($admin['gambar']) && file_exists('img/'.$admin['gambar'])) ? 'img/'.htmlspecialchars($admin['gambar']) : 'https://cdn-icons-png.flaticon.com/512/149/149071.png'; ?>"
                     class="img-preview rounded-circle mb-3">
                <p class="small text-muted">Klik "Ubah" untuk mengedit dan memilih foto baru.</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <footer class="sticky-footer bg-white">
      <div class="container my-auto"><div class="copyright text-center my-auto"><span>Copyright &copy; 2023. Great Group Indonesia</span></div></div>
    </footer>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    btnBatal.addEventListener('click', function(){
        // reload untuk mengembalikan nilai lama
